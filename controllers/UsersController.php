...
class UsersController
{
    ...
    public function addUser()
    {
        if ($_POST['email'] != '' && $_POST['password'] != '') {
            ...
            $users->insert();
            if ($users->id !== false) {
               $_SESSION['ok'] = 'Пользователь добавлен, перейдите  <a href="?ctrl=users&action=showProfile&id=' . $users->id.'"> на  страницу для просмотра профиля </a>. Id пользователя:' . $users->id;
            }
        } elseif ($_POST['email'] == '' || $_POST['password'] == '') {
            $_SESSION['errors'] = 'Не введены обязательные данные';
        }
        header('Location: http://newssite/index.php?ctrl=users&action=addUser');
        exit;
    }
    ...
    public function findUser()
    {
        ...
        header('Location: http://newssite/index.php?ctrl=users&action=findUser');
        exit;
